...
class POJ(object):
    def __init__(self):
        ...
        self.base_url="http://poj.org/status?top="
    ...
    def initsqlit(self):
        path = os.getcwd()
        path = os.path.join(path, "POJ_submit.db")
    ...
    def get_runid(self):
        ...
        runid=soup.find_all("tbody")[4].find_all("tr")[1].find_all("td")[0].text
        return runid

    #进行一次更新，数据导入数据库
    def update(self,runid):
        soup=self.get_soup(runid)
        sub_list = soup.find_all("tbody")[4].find_all("tr")
        for submit in sub_list[1:]:
            submit_info = submit.find_all("td")
            submit_resu = submit_info[3].text
            if (submit_resu == "Accepted"):
                submit_proid = submit_info[2].text
                submit_runid = submit_info[0].text
                submit_memory = submit_info[4].text
                submit_times = submit_info[5].text
                submit_codelen=submit_info[7].text
                submit_time=submit_info[8].text
    ...
    def main_work(self):
        ...
        while(1):
            now_id = self.get_runid()
            while (int(now_id) - int(self.runid) < 20):
                time.sleep(10)
                now_id = self.get_runid()
            self.runid=str(int(self.runid)+20)
            try:
